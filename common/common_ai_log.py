...
def init_ai_log_db(db_path):
    ...
    If the shared db_path is not writable, falls back to ~/.batchRun/db/ai/.
    """
    ...
    try:
        ...
        ai_db_dir = os.path.expanduser('~/.batchRun/db/ai')
        os.makedirs(ai_db_dir, exist_ok=True)
...
def get_tool_preferences(db_file, user_message, min_count=2):
    ...
    Returns a list of tool preference hints like "df -h (used in 80% of similar cases)".
    """
    if not os.path.exists(db_file):
        return []
...
class InsightGeneratorThread(QThread):
    ...
    def _generate_insight(self):
        ...
        prompt = f"""Based on this solved HPC cluster administration conversation, write ONE concise insight (under 80 chars) that would help resolve similar future questions. Focus on the key diagnosis step or solution pattern.
...
class AiReportThread(QThread):
    ...
    def _generate_analysis(self):
        ...
        for start in range(0, len(summaries), batch_size):
            ...
            prompt = f"""请分析以下 {len(batch)} 条集群运维AI助手的对话记录，生成一份分类分析报告。
